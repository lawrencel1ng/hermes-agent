/**
 * Portal Calls Page Server
 * Validates tenant has a voice platform before rendering call history UI
 */
...
export const load: PageServerLoad = async ({ request, cookies }) => {
  ...
  try {
    ...
    if (platformResult.rows.length === 0) {
      logger.warning('Portal calls page accessed without voice platform configured', {
        tenantId
      }, { source: 'portal/calls' });
    ...
    logger.error('Failed to validate voice platform for portal calls', err, { source: 'portal/calls' });
