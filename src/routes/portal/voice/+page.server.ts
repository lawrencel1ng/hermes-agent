...
export const load: PageServerLoad = async ({ request, cookies }) => {
  ...
  try {
    ...
    if (platformResult.rows.length === 0) {
      logger.warning('Portal voice page accessed without voice platform configured', {
        tenantId
      }, { source: 'portal/voice' });
    ...
    logger.error('Failed to validate voice platform for portal voice', err, { source: 'portal/voice' });
